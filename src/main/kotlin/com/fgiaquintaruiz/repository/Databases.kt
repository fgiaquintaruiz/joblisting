...
import com.fgiaquintaruiz.config.AwsSecretsHandler
...
fun Application.connectToMongoDB(): MongoDatabase {
    ...
    val secret = AwsSecretsHandler.getSecret()
    val user = environment.config.tryGetString("db.mongo.user") ?: secret.mongoDbUser
    val password = environment.config.tryGetString("db.mongo.password") ?: secret.mongoDbPassword
    ...
    logger.trace("user: {}, password: {}, host: {}, port: {}", user[0], password[0], host, port)
    val credentials = user.let { userVal ->
        password.let { passwordVal -> "$userVal:$passwordVal@" } }
    val uri = when(host) {
        "127.0.0.1" ->"mongodb://$credentials$host:$port/?maxPoolSize=$maxPoolSize&w=majority"
        else -> {
            "mongodb+srv://$credentials$host/?retryWrites=true&w=majority&appName=Cluster0"
        }
    }
